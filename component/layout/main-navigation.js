...
import { ShoppingCart as Cart, Menu as Hamburger } from "lucide-react";
...
const MainNavigation = ({ categories, products }) => {
  ...
  if (cartCtx.carts.length > 0) {
    content = (
      <div onClick={deleteCartHandler} className={classes.deleteCart}>
        <TrashIcon />
      </div>
    );
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      const isClickOutsideCategoryDropdown =
        (!categoryDropdownDesktopRef.current ||
          !categoryDropdownDesktopRef.current.contains(event.target)) &&
        (!categoryDropdownMobileRef.current ||
          !categoryDropdownMobileRef.current.contains(event.target));
      ...
      const isClickOutsideSearchDropdown =
        (!searchDropdownDesktopRef.current ||
          !searchDropdownDesktopRef.current.contains(event.target)) &&
        (!searchDropdownMobileRef.current ||
          !searchDropdownMobileRef.current.contains(event.target));
    ...
    <div className={classes.nav}>
      <div className={classes.desktop}>
        ...
        <div className={classes.items}>
          ...
          <div>
            <p>
              <a a href={`/product_categories/food and soup`}>
                Sellers Deal
              </a>{" "}
            </p>
          </div>
          <div>
            <p>
              <a a href={`/product_categories/home appliences`}>
                Home Favourites{" "}
              </a>
            </p>
          </div>
